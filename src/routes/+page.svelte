<div class="flex items-center justify-center min-h-screen bg-slate-white">
	<div class="container mx-auto">
		<div class="max-w-md mx-auto">
			<div class="text-center">
				<img src="Mwenzi.png" class="h-auto max-w-full mb-5 duration-500 hover:-translate-y-48" alt="..." />
			</div>

			<div class="flex flex-col justify-center items-center">
				<!-- Button -->
				<a href="/Teacher" id="Teacher">
					<button
						type="button"
						class="flex bg-[#2ea44f] text-white border-2 border-transparent rounded-3xl px-28 py-1 mb-4 w-full hover:bg-[#1e7d3f] duration-300 hover:scale-105 "
					>
						<div class="m-auto">
							<div class="flex items-center justify-start">
								<img src="teacher.png" class="h-10 pr-2 pt-1 " alt="..." />
								
								<span class="font-medium">I'm a Teacher</span>
							</div>
						</div>
					</button>
				</a>

				<a href="/Student" id="Student">
					<button
						type="button"
						class="flex bg-[#2ea44f] text-white border-2 border-transparent shadow-lg rounded-3xl px-28 py-1 mb-4 w-full hover:bg-[#1e7d3f] duration-300 hover:scale-105"
					>
						<div class="m-auto">
							<div class="flex items-center justify-start">
								<img src="student.png" class="h-10 pr-2 pt-1" alt="..." />
								<span class="font-medium">I'm a Student</span>
							</div>
						</div>
					</button>
				</a>

				<a href="/Admin" id="Admin">
					<button
						type="button"
						class="flex bg-[#2ea44f] text-white border-2 border-transparent shadow-lg rounded-3xl px-28 py-1 mb-4 w-full hover:bg-[#1e7d3f] duration-300 hover:scale-105"
					>
						<div class="m-auto">
							<div class="flex items-center justify-start">
								<img src="admin.png" class="h-10 pr-2" alt="..." />
								<span class="font-medium">I'm an Admin</span>
							</div>
						</div>
					</button>
				</a>
			</div>
		</div>
	</div>
</div>
